        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <h3 class="mb-0">Modifier la transaction</h3>
                </div>
                            ...
                            <select class="form-select @error('site_id') is-invalid @enderror" 
                                ...
                                @foreach($sites as $site)
                                    <option value="{{ $site->id }}" 
                                        {{ old('site_id', $transaction->site_id) == $site->id ? 'selected' : '' }}>
                                        {{ $site->name }}
                                    </option>
                                @endforeach
                            </select>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    typeSelect.addEventListener('change', updateAmountColor);
    updateAmountColor(); // Appliquer la couleur initiale
});
</script>
@endpush 